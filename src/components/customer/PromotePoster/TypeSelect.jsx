...
function TypeSelect({ input, setInput }) {
  return (
    <div className="flex gap-1 [&_*]:box-content box-content w-fit">
      <div className={`border rounded-lg bg-slate-500`} style={{ width: BOX30 + "px" }}></div>
      <div className="flex flex-col gap-1">
        <button
          onClick={() => setInput((prev) => ({ ...prev, type: 1 }))}
          className={`grid place-items-center  aspect-[2/1] ring-blue-500 border rounded-lg hover:bg-slate-100 duration-150 ${
            input.type === 1 ? "ring-2" : ""
          }`}
