...
export default function EditCategory({ show, hide, category, afterLeave }) {
  ...
  useEffect(() => {
    setInput((prev) => ({ colorInput: category?.color || "#3b82f6", color: category?.color || "#3b82f6", name: category?.name || "" }));
  }, [category]);
  ...
  return (
    ...
    >
      <div className="flex justify-between bg-gray-50 pb-5 px-2">
        <h3 className="text-lg font-medium text-gray-900">Modifier catégorie</h3>
        <button
          type="button"
          onClick={() => {
            hide();
      ...
      <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            ref={inputRef}
          />
          <p className="mb-1 font-medium text-lg text-sky-900">Color:</p>
