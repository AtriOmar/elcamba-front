...
function Navbar() {
  ...
  const { setMobileNavbarOpen, setFilterSidebarOpen } = useUIContext();
    ...
    <header className="sticky top-0 left-0 z-[20] px-4 py-1.5 scr800:py-0 bg-white shadow-md">
      ...
      <div className="flex items-center justify-between max-w-w1300 m-auto py-1">
        <div className="flex gap-4">
          <button
            className={`${location?.pathname === "/" ? "min-[1350px]:hidden" : location?.pathname?.startsWith("/products") ? "" : "scr1000:hidden"} block`}
            onClick={() => {
